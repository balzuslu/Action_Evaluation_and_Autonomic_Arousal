...
my_figure_theme <- theme_apa(base_size = 11) +
  theme(legend.position = "top", axis.ticks.x = element_blank(), axis.title.y = element_text(vjust = -0.5))
...
# For SCR analysis, exclude exclude inhibited responses, missing responses, responses with wrong key, trials with    
...
descriptive_statistics_scr_z <- summarySEwithinO(
  ...
  dplyr::mutate(
    ci_scr = paste0("[", iscr_gng_resp_z - ci, ", ", iscr_gng_resp_z + ci, "]")
  ) %>%
...
```{r descriptive-statistics-plot, fig.cap = "Note. Standardized SCR is shown as a function of response type in the go/no-go task. Error bars represent 95% confidence intervals adjusted for within-participant designs as described by (Morey, 2008)"}
...
plot_scr <- ggplot(descriptive_statistics_scr_z, 
  ...
  geom_signif(
    y_position = c(0.48,0.43), xmin = c(1,2), xmax = c(3,3),
...
<br><br>
This table corresponds to the results section "Skin Conductance Response" in the paper and Table S5 in the Supplemental Material.  
...
```{r SCR-priming-LMM-final-model, cache = TRUE}

#### 3) Run final model without random terms explaining zero variance (i.e., remove all interaction terms involving the factor SCR)
...
```{r SCR-priming-LMM-nested-model, cache = TRUE}
...
# Display results (fixed effects) - but only show follow-up for interactions that were significant
tab_model(LMM_scr_priming_nested,
  dv.labels = "RT", pred.labels = labels, show.stat = TRUE, show.icc = FALSE, show.r2 = FALSE, 
  show.re.var = FALSE, show.ngroups = FALSE, string.est = "b", string.stat = "t value", 
  string.ci = "95 % CI", string.p = "p value",  p.val = "satterthwaite", 
  rm.terms = c(
    "gng_response_typeSH:word_valence2-1:iscr_gng_resp_sqrt_z",
    "gng_response_typeFH:word_valence2-1:iscr_gng_resp_sqrt_z",
    "gng_response_typeFA:word_valence2-1:iscr_gng_resp_sqrt_z"
  )
)
```
<br>
...
#### Plot PES - SCR Finding
<br>
The plot shows the predicted values (marginal effects) for the response type x SCR interation of the LMM. 
<br><br>

##### Model on inverse RTs (original)

```{r plot-pes-scr-original, cache = TRUE, fig.width = 4, fig.cap = "Error bars indicate 95% confidence intervals."}

# Original LMM doesn't work with plot_model: plot_model does not like interactions in random structure
# Thus, I have to delete double bar syntax (model does still converge, is not singular)
# Also: respecify model to have factors as fixed effects, not contrasts: needed to plot all levels of a factor, not only a specific contrast
...
plot_model(LMM_scr_priming_plot, type = "pred", terms = c("iscr_gng_resp_sqrt_z", "gng_response_type"), legend.title = "Response Type") + 
  labs(title = NULL, x = "Standardized square root-transformed SCR (µS)", y = "Predicted values of inverse RT (-1000/RT in ms)") +
  theme_apa(base_size = 11) +
  theme(legend.position = "bottom", axis.title.y = element_text(vjust = -0.5)) +
...
# ggsave("figure_scr_pes.tiff",width = 11, height = 13 , units = "cm", dpi=600, compression = "lzw")

# Possibility 2: specify model as LMM_scr_priming_final (but specify factors as fixed effects, leave contrasts as random effects) but delete interactions from random structure (gives almost same result, a bit narrower CIs, but model results are more different to original model)

# Possibility 3: I could specify LMM_scr_priming_plot with raw values to see how the plot would like on the raw RT scale (other back-transformation of estimates is not possible; only predictors can be back-transformed: https://github.com/strengejacke/sjPlot/issues/544)
```
<br><br>

##### Model on raw RTs

This is just to see how the plot would like on the raw RT scale. The model is only approximately comparable to the original one (as it models the raw RT values, not the inverse-transformed values). For reporting, the plot based on the original model should be used.

```{r plot-pes-scr-raw, cache = TRUE, fig.width = 4}

# I could plot LMM with raw values, other back transformation of estimates is not possible (only predictors can be backtransformed)
LMM_scr_priming_plot_raw <- lmer(word_rt ~ gng_response_type * word_valence * iscr_gng_resp_sqrt_z + 
                                 (1 + gng_response_type * word_valence + iscr_gng_resp_sqrt_z | participant_id) +
                                 (1 + gng_response_type                + iscr_gng_resp_sqrt_z | word),
                               data = single_trial_data_scr,
                               REML = TRUE,
...
plot_model(LMM_scr_priming_plot_raw, type = "pred", terms = c("iscr_gng_resp_sqrt_z", "gng_response_type"), legend.title = "Response Type") + 
  labs(title = NULL, x = "Standardized square root-transformed SCR (µS)", y = "Predicted values of RT (ms)") +
  theme_apa(base_size = 11) +
  theme(legend.position = "bottom", axis.title.y = element_text(vjust = -0.5)) +
  scale_y_continuous(breaks=seq(450, 900, 150), limits = c(450, 900), expand = c(0,0)) +
  scale_x_continuous(breaks=seq(-4, 6, 2), limits = c(-4, 6), expand = c(0,0))


# ggsave("figure_scr_pes_raw.tiff",width = 11, height = 13 , units = "cm", dpi=600, compression = "lzw")
```
<br><br><br>
...
# Attach the model matrix (only 3 new columns) to the dataframe
single_trial_data_scr[, (ncol(single_trial_data_scr) + 1):(ncol(single_trial_data_scr) + 6)] <- model_matrix
...
```
SCR in trials beginning with a red compared to those with a white fixation cross did not differ. 
<br><br><br>
