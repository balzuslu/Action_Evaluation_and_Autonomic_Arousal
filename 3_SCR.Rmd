...
This figure corresponds to Figure 4 in the manuscript. The plot shows the predicted values (marginal effects) for the response type x SCR interation of the LMM. 

```{r plot-pes-scr-original, cache = knitr_cache_enabled, fig.cap = "Note. Estimated interaction effects of SCR and response type in the go/no-go task on word categorization RT. RT values are displayed on an inverse scale, standardized SCR values on a square root scale. The slope is significant only for false alarms, indicating that following incorrect responses, an enhanced SCR is associated with slower responses to words and hence more post-error slowing. Shaded areas represent 95% confidence intervals."}
