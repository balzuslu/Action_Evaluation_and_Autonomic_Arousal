...
```{r plot-pes-scr-original, cache = TRUE, fig.width = 4, fig.cap = "Note. Error bars indicate 95% confidence intervals."}

# Respecify LMM without interactions stated with ":" in random structure (because plot_model does not like that) and to have 
# fixed effects as factors, not contrasts (needed to have all factor levels in the plot)
# Double bar syntax for zero-correlation parameter model won't work anymore (model still converges and fit is not singular)
...
plot_model(LMM_scr_priming_plot, 
           type         = "pred", 
           terms        = c("iscr_gng_resp_sqrt_z", "gng_response_type"), 
           legend.title = "Response Type", 
           colors       = c("#56B4E9", "#009E73", "#F0E442")) +
  labs(title = NULL, 
       x     = "Standardized square root-transformed SCR (µS)", 
       y     = "Predicted values of inverse RT (-1000/RT in ms)") +
  my_figure_theme +
