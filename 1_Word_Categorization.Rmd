...
```{r descriptive-statistics-plot, fig.cap = "Note. (A) RT and (B) accuracy in the word categorization task are shown as a function of previous response type and word valence. Error bars represent 95% confidence intervals adjusted for within-participant designs as described by (Morey, 2008)"}
...
FH_time_sd   <- round((sd(FH_time$FH_1)   + sd(FH_time$FH_2)   + sd(FH_time$FH_3))   / 3, digits = 2)
```
...
Fixed effects were coded using sliding difference contrasts, such that the intercept reflects the grand mean across all conditions and differences in means of adjacent factor levels (e.g., false alarm minus fast hit) are tested. 
...
```{r RT-LMM-final-model, cache = TRUE}
...
```{r RT-GLMM-final-model, cache = TRUE}
...
```{r RT-LMM-nested-model, cache = TRUE}
...
```{r RT-GLMM-nested-model, cache = TRUE}
...
# Check model output
summary(GLMM_acc_red1) # Model does converge but singular fit
...
```{r accuracy-GLMM-final-model, cache = TRUE}
...
<br><br>
This table corresponds to Table 2 in the paper. 
```{r accuracy-GLMM-nested-model, cache = TRUE}
